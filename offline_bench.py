...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Run baby-vllm offline DP/TP smoke validation or throughput benchmark.",
        epilog=(
            "Examples:\n"
            "  DP smoke:      python offline_bench.py --model /path/to/Qwen3-0.6B --mode smoke --dp-sizes 2 --tp-size 1\n"
            "  TP smoke:      python offline_bench.py --model /path/to/Qwen3-0.6B --mode smoke --dp-sizes 1 --tp-size 2\n"
    ...
    parser.add_argument(
        "--dp-sizes",
        default=None,
        help="Data parallel size for this process. Use one value only, for example 1 or 2.",
    )
    parser.add_argument(
        "--tp-size",
    ...
    parser.add_argument("--max-num-sequences", type=int, default=256)
    parser.add_argument("--max-prefill-tokens-per-step", type=int, default=8192)
    parser.add_argument("--max-prefill-chunk-size", type=int, default=4096)
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if len(args.dp_sizes) != 1:
        raise ValueError("--dp-sizes accepts exactly one value per process.")
    if args.num_seqs <= 0:
        raise ValueError("--num-seqs must be positive.")
    if args.min_input_len <= 0 or args.max_input_len <= 0 or args.min_input_len > args.max_input_len:
        raise ValueError("Input length bounds must be positive and ordered.")
    if args.min_output_len <= 0 or args.max_output_len <= 0 or args.min_output_len > args.max_output_len:
...
def format_results_table(results: Sequence[RunResult]) -> str:
    ...
    for result in results:
        block = [
            "baby-vllm offline DP/TP benchmark",
            "",
...
def run_single_dp_size(args: argparse.Namespace, workload: Workload, dp_size: int) -> RunResult:
    ...
    preflight_cuda_devices(dp_size=dp_size, tp_size=args.tp_size, device_ids=selected_device_ids)
    ...
    try:
        ...
        outputs, metrics = llm.generate(workload.clone_prompts(), workload.clone_sampling_params())
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        print(f"baby-vllm offline DP/TP {args.mode}")
        print(
