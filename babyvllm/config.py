...
class Config:
    ...
    def __post_init__(self):
        if not os.path.isdir(self.model):
            raise ValueError(
                ...
                "Please check that the path is correct and the model files are present."
            )
    ...
    def _validate_qwen3_tensor_parallel_config(self):
        ...
        local_num_kv_heads = self._validate_tp_divisibility(
            ...
            tp_size=tp_size,
            extra=(
                "KV-head replication is not supported by baby-vllm-basic-online; "
        ...
        if local_num_kv_heads <= 0:
            raise ValueError(
                ...
                "supported by baby-vllm-basic-online."
            )
