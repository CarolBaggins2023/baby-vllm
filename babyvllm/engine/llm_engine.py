...
def worker_process(config, rank, event):
    """Entrance of one TP worker process."""
    import os
    import sys
...
# Offline DP coordinator/worker Pipe protocol:
...
def data_parallel_offline_worker_process(model: str, config_kwargs: dict, connection: Connection):
    """Run one full offline LLMEngine replica for a DP rank."""
    import os
    import sys
    ...
    try:
        ...
        while True:
            ...
            if not isinstance(message, tuple) or not message:
                connection.send(("error", None, rank, "Malformed offline DP worker message."))
                continue
            ...
            if command != "generate" or len(message) != 5:
                connection.send(("error", None, rank, f"Unknown offline DP worker command: {command!r}"))
                continue
...
class LLMEngine:
    def __init__(self, model, **kwargs):
        """Create either a single offline replica or an offline DP coordinator."""
    ...
    def _init_single_replica(self):
        """Initialize one local engine replica, including local TP workers."""
        config = self.config
    ...
    def _worker_config_kwargs(self, rank: int) -> dict:
        """Build config kwargs for one offline DP worker's single-replica engine."""
        config_kwargs = self.config.worker_config_kwargs(rank)
        config_kwargs.pop("model", None)
        config_kwargs["num_kvcache_blocks"] = -1
        return config_kwargs

    def _init_data_parallel_workers(self):
        """Launch one full offline engine replica process per DP rank."""
        ctx = mp.get_context("spawn")
        ...
        for rank in range(self.config.data_parallel_size):
            ...
            process = ctx.Process(
                target=data_parallel_offline_worker_process,
                args=(self._model, self._worker_config_kwargs(rank), child_conn),
                name=f"babyvllm_offline_dp_rank_{rank}",
            )
            process.start()
            child_conn.close()
    ...
    def _shutdown_data_parallel_workers(self):
        """Stop offline DP worker processes and close Pipe handles."""
    ...
    def _validate_sampling_params(self, sampling_params: SamplingParams):
        ...
        if (
            not isinstance(sampling_params.max_tokens, int)
            or isinstance(sampling_params.max_tokens, bool)
            or sampling_params.max_tokens <= 0
        ):
    ...
    def _prepare_request(
        ...
        prompt_token_ids = self._coerce_prompt_token_ids(
            prompt=prompt,
    ...
    def get_stats(self) -> dict[str, dict[str, int]]:
        ...
        if getattr(self, "_is_data_parallel_coordinator", False):
            return {"data_parallel": {"workers": len(self.dp_processes)}}
    ...
    def generate(
        ...
        while not self.scheduler.is_finished():
            ...
            memory_usages.append(memory_stats["allocated_bytes.all.current"]/(1024**2))
        ...
        for seq_id in sequence_start_times:
            if seq_id in first_token_times:
                ...
                if seq_id in sequence_token_counts and sequence_token_counts[seq_id] > 0:
                    ...
                    tpot = total_gen_time/sequence_token_counts[seq_id]
    ...
    def _recv_data_parallel_message(self, rank: int, request_id: int | None):
        ...
        while True:
            ...
            if not process.is_alive():
                suffix = "" if request_id is None else f" while processing request {request_id}"
    ...
    def _generate_data_parallel(
        ...
        """Dispatch an offline generate batch across DP workers and restore input order."""
        sampling_params_list = self._normalize_generation_inputs(prompts, sampling_params)
        if not prompts:
