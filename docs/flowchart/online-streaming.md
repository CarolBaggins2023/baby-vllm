...
sequenceDiagram
  ...
  participant Loop as "_run_engine_loop()"
  ...
  Tracker->>Loop: "new_requests_event.set()"
  Loop->>Tracker: "get_new_and_aborted_requests()"
  Loop->>Engine: "add Sequence to scheduler"
  Loop->>Engine: "schedule()"
  Loop->>Engine: "run_scheduled(decode_sequences)"
  Engine-->>Loop: "seq token deltas"
  Loop->>Tracker: "process_step_outputs(RequestOutput)"
