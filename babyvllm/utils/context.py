...
class Context:
    ...
    # `cu_seqlens_q` is the cumulative sequence lengths for uncached query tokens.
    ...
    # `max_seqlen_q` is the maximum uncached query length.
    # For example, in the above example, the longest sequence is Sequence 2, with length 5,
    # so `max_seqlen_q` is 5.
    max_seqlen_q: int = 0
    # `cu_seqlens_k` is the cumulative sequence lengths for all key tokens, including cached tokens.
    ...
    # `max_seqlen_k` is the maximum key length, including cached tokens.
    max_seqlen_k: int = 0
