...
class AsyncLLMEngine:
    ...
    def __init__(self, model: Optional[str] = None, engine: Optional[LLMEngine] = None, **kwargs):
        ...
        if engine is not None:
            ...
            if (
                ...
                self._model = model
                self._config_kwargs = self.config.worker_config_kwargs(0)
                self._config_kwargs.pop("model", None)
                self._config_kwargs["num_kvcache_blocks"] = -1
        ...
        self._data_parallel_reader_tasks: list[asyncio.Task] = getattr(self, "_data_parallel_reader_tasks", [])
        self._data_parallel_request_streams: dict[int, AsyncStream] = getattr(self, "_data_parallel_request_streams", {})
        self._data_parallel_request_to_rank: dict[int, int] = getattr(self, "_data_parallel_request_to_rank", {})
        self._data_parallel_next_rank = getattr(self, "_data_parallel_next_rank", 0)
    ...
    def _shutdown_data_parallel_workers(self):
        ...
        for connection, process in zip(getattr(self, "dp_connections", []), getattr(self, "dp_processes", [])):
            if process.is_alive():
                try:
    ...
    def get_stats(self) -> dict[str, dict[str, int]]:
        ...
        if self._is_data_parallel_coordinator:
            return {
                "data_parallel": {
                    "workers": len(getattr(self, "dp_processes", [])),
                    "live_workers": len(self._live_data_parallel_ranks),
                }
            }
