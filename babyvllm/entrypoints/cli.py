...
Examples:
  ...
  babyvllm-server --model /data/models/Qwen2-0.5B-Instruct --log-level debug
    ...
    #
    # All kwargs passed to AsyncLLMEngine.__init__(), which forwards them
    # to LLMEngine.__init__() → Config.__init__().
    # =====================================================================
    ...
    engine_kwargs = {
        ...
        # Later, cli.py reads them back from engine.config instead of raw
    ...
    # =====================================================================
    #
    # Why import here instead of at module top?
    #   Importing babyvllm triggers:
    #     - torch import (GPU driver initialization, CUDA context creation)
    #     - transformers import (model config parsing)
    #   These are expensive operations (>1 second). Deferring to after
    #   argument parsing means:
    #     - "babyvllm-server --help" runs instantly
    #     - Invalid arguments are caught before GPU initialization
    #     - Clearer error messages (no torch stack traces for simple typos)
    # =====================================================================
