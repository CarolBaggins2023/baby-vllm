...
class CompletionStreamResponse(OpenAIBaseModel):
    ...
    Unlike CompletionResponse, stream chunks do NOT include usage (added
    in a separate final chunk by some OpenAI-compatible servers).
    """
    id: str
...
class ChatCompletionStreamResponse(OpenAIBaseModel):
    ...
    Usage is only included in the FINAL chunk (when finish_reason is set).
    """
    id: str
...
async def _stream_completion(
    ...
    try:
        async for output in _engine.generate(prompt, sampling_params):
            ...
            # SSE format: "data: <json>\n\n"
            # model_dump_json() is Pydantic v2's JSON serialization method
