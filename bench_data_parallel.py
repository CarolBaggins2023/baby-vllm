...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Minimum generated-token limit per request.",
    )
    parser.add_argument(
        "--max-output-len",
        type=int,
        default=32,
        help="Maximum generated-token limit per request.",
    )
    ...
    parser.add_argument(
        "--enforce-eager",
        action="store_true",
        help="Force eager execution. Omit to allow CUDA graph mode.",
    )
    parser.add_argument(
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if len(args.dp_sizes) != 1:
        raise ValueError(
            ...
            "commands for each DP size, for example '--dp-sizes 1' and then "
            "'--dp-sizes 2', to avoid same-process CUDA memory reuse."
        )
    if args.num_seqs <= 0:
...
def run_benchmark(args: argparse.Namespace) -> list[RunResult]:
    ...
    dp_size = args.dp_sizes[0]
    return [run_single_dp_size(args, workload, dp_size)]
